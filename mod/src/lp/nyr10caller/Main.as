...
import lp.nyr10caller.CharacterSubscription;

class lp.nyr10caller.Main {
    ...
    private function Add(id:ID32) {
		...
		} else if (name == "The Unutterable Lurker" && dynel.GetStat(421)) { // 421 == 27 No idea what this stat is but it seems to be specific to lurker
			m_dynels.push(dynel);
    ...
    private function PlayFieldChanged(newPlayfield:Number) {
		//for (var i in m_dynels) {
            //Remove(m_dynels[i].GetID());
        //}
		Log.Error("Nyr10Caller", "PlayFieldChanged: " + newPlayfield + "|" + LDBFormat.LDBGetText("Playfieldnames", newPlayfield));
