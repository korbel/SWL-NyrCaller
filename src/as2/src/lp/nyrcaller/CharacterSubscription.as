...
class lp.nyrcaller.CharacterSubscription {
    ...
    private function Subscribe(shouldLogCommands:Boolean) {
        ...
        Log.Error("NyrCaller", "DynelSubscribed: " + m_character.GetID() + "|" + m_character.GetName());
    }
    ...
    private function Unubscribe() {
        ...
        Log.Error("NyrCaller", "DynelUnsubscribed: " + m_character.GetID());
    }
    ...
    private function StatChanged(statID:Number) {
        if (!m_statFilter || ArrayUtils.Contains(m_statFilter, statID)) {
            Log.Error("NyrCaller", "StatChanged: " + m_character.GetID() + "|" + statID + "|" + m_character.GetStat(statID));
        }
    }
    ...
    private function CharacterDied() {
        Log.Error("NyrCaller", "CharacterDied: " + m_character.GetID());
    }

    private function CharacterAlive() {
        Log.Error("NyrCaller", "CharacterAlive: " + m_character.GetID());
    }
    ...
    private function BuffAdded(buffId:Number) {
        if (!m_buffFilter || ArrayUtils.Contains(m_buffFilter, buffId)) {
            ...
            Log.Error("NyrCaller", "BuffAdded: " + m_character.GetID() + "|" + buffId + "|" + tooltip.m_Title);
        }
    }

    private function BuffRemoved(buffId:Number) {
        if (!m_buffFilter || ArrayUtils.Contains(m_buffFilter, buffId)) {
            Log.Error("NyrCaller", "BuffRemoved: " + m_character.GetID() + "|" + buffId);
        }
    }

    private function BuffUpdated(buffId:Number) {
        if (!m_buffFilter || ArrayUtils.Contains(m_buffFilter, buffId)) {
            Log.Error("NyrCaller", "BuffUpdated: " + m_character.GetID() + "|" + buffId);
        }
    }
    ...
    private function InvisibleBuffAdded(buffId:Number) {
        if (!m_buffFilter || ArrayUtils.Contains(m_buffFilter, buffId)) {
            var tooltip:TooltipData = TooltipDataProvider.GetBuffTooltip(buffId, m_character.GetID());
            Log.Error("NyrCaller", "InvisibleBuffAdded: " + m_character.GetID() + "|" + buffId + "|" + tooltip.m_Title);
        }
    }

    private function InvisibleBuffUpdated(buffId:Number) {
        if (!m_buffFilter || ArrayUtils.Contains(m_buffFilter, buffId)) {
            Log.Error("NyrCaller", "InvisibleBuffUpdated: " + m_character.GetID() + "|" + buffId);
        }
    }
    ...
    private function CommandStarted(name:String, progressBarType:Number, uninterruptable:Boolean) {
        Log.Error("NyrCaller", "CommandStarted: " + m_character.GetID() + "|" + name);
    }

    private function CommandEnded() {
        Log.Error("NyrCaller", "CommandEnded: " + m_character.GetID());
    }

    private function CommandAborted() {
        Log.Error("NyrCaller", "CommandAborted: " + m_character.GetID());
    }
