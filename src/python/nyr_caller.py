...
log_line_pattern = re.compile(r'\[(?P<date_string>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})Z.+?\].*?Scaleform\.NyrCaller - (?P<event_type>\w+)(: (?P<params>.+))?')
...
def event_buff_added(character_id, buff_id, buff_name):
    if buff_name == 'Inevitable Doom':
        ...
        if character_id not in dynels:
            error("Players don't seem to be registered! Re-enter the raid or type /reloadui in the chat")
...
def main():
    debug("Agnitio NYR Caller bot started")
    reset_game_state()
